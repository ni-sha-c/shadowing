...
\reviewerOne{
At the same time, families of periodic solutions have been well-studied for some problems}\cite{henon3body}.\reviewerOne{The set of all unstable periodic orbits have been recognized as the ``skeleton'' of the chaotic attractor \cite{cvitanovic1} forming a dense subset of the attractor and providing an alternative characterization of the SRB distribution \cite{llave}. This implies that ergodic/ensemble averages with respect to the SRB distribution are also weighted averages along the set of periodic orbits.} \reviewerOne{The significance of the set of unstable periodic orbits extends even further as it can be used to compute dynamical invariants such as entropy and Lyapunov dimension \cite{grebogi1}\cite{cvitanovic1}. A quantity of great mathematical interest is the dynamical determinant (see \cite{baladi-book} for a recent review), which is computed as a weighted sum over all periodic orbits. The zeros of the dynamical determinant have a one-to-one correspondence with the slowest decay rates of correlations between observables \cite{baladi-book}. This connection has been exploited both in mathematical as well as physical studies of the distribution of energy levels and resonances in quantum scattering \cite{semyonZworski}\cite{berry}\cite{cvitanovic2005chaos}. Furthermore, theoretical and numerical studies of unstable periodic orbits have been crucial to better understanding transitions to chaos \cite{kreilos} and cataclysmic changes in the shape and position of the attractor in response to small parameter changes, known as attractor crises \cite{grebogi1}\cite{lucariniCrisis}, which are sometimes due to the emergence of tangencies between the stable and unstable manifolds along periodic orbits.}
...
of equations, we first make a closed form approximation of the Lorenz map (see section \ref{sec:lorenzMapApprox} of the Appendix). Except at rare parameter values, the Lorenz system does not have 
...
examples in which numerical error due to computation in double-precision floating point arithmetic causes significant change in the stationary probability distribution on the attractor. \reviewerOne{Some authors} \cite{cns2}\cite{cns3} \reviewerOne{resort to Clean Numerical Simulations to tackle scenarios that demand highly accurate long-term prediction of chaotic orbits. When using double-precision arithmetic however, these works}\cite{cns}\cite{cns1}\cite{cns2}\cite{cns3}\reviewerOne{show that large errors result in statistical measures such as power spectra and spatio-temporal correlation functions.} The plucked tent map adds to this list of examples by demonstrating a mechanism to produce an extreme non-smooth response (Appendix section \ref{sec:pluckedTentMap}). 

\reviewerTwo{Here, we raise questions on the statistical consistency on numerical simulations of chaotic systems because they are traditionally interpreted as being shadowed by a true solution. }\reviewerTwo{Since shadowing solutions can be nonphysical, so can numerical solutions that lie close to them at all times, and hence have a long-term statistical distribution close to their shadowing counterparts.} \reviewerTwo{This argument holds even when numerical solutions are interpreted as perturbed solutions to the true dynamics that produce the same or a nearby attractor.}\reviewerTwo{The probability distribution on the attractor, measured by the amount of time a solution spends in each location on the attractor, can be drastically different for a numerical solution than a typical solution of the dynamics.}
...
The code and data to generate the figures can be found on Github \cite{supp}. The code can be found under the \verb+code+ subdirectory inside which section-wise code is separated into further subdirectories. The data used to plot the figures can be found under \verb+data+. The files referred to in this section can be found in the appropriate subdirectory under \verb+code+. 
