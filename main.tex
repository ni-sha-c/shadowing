...
%             \emph{Present address:} of F. Author  %  if needed
           Center for Computational Science and Engineering, MIT, Cambridge, MA 02139, USA. Corresponding author email: \email{nishac@mit.edu}  \email{qiqi@mit.edu}. }
...
\begin{abstract}
In this work, we show that a small parameter perturbation can lead to a dramatic change in the statistics of a chaotic system. This theoretical observation implies that chaotic numerical simulations are not guaranteed to emulate the true  able to c ability of chaotic numerical simulations to capture the true long-term behavior of underlying governing equation. the failure of numerical simulations to capture the long-term statistical behavior of the governing equations. whether  the fidelity of chaotic numerical simulations to the underlying physics. 
...
When the slightly different governing equations are uniformly hyperbolic, it has been long know that infinitely 
...
solutions may still exist \cite{grebogi}\cite{hammel} for finite amounts of time.
...
Because of numerical and modeling errors, we expect there will be a small difference between the
true governing physics and the equations solved on a computer. With time evolution, by nature of chaos, any such small difference grows exponentially. Justifiably, this calls into question the fidelity of numerical solutions to chaotic systems, such as turbulent flows, to the true physics.  The existence of shadowing solutions
is used to argue for the veracity of numerical solutions.
