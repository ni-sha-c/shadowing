...
\begin{figure}[H]
  \centering
    ...
    \\ \vspace{0.02\textwidth}
     \caption{Upper-left: the tilted tent map (\ref{tent_tilted}) for $s=0, 0.1, 0.2, 0.3, 0.4,$ and 0.5.
    Upper-right: the conjugate map $\tilde{h}_s$ between the tent map (Eq. \ref{tentmap})
...
\begin{figure}[H]
   ...
   \caption{
     Lower-left: the density of a trajectory satisfying (\ref{tentmap})
    that shadows a random trajectory satisfying (\ref{tent_tilted}) for $s=0.1$.
    Lower-right: the density of a trajectory satisfying (\ref{tentmap})
    that shadows a random trajectory satisfying (\ref{tent_tilted}) for $s=0.5$.
    }
    \label{fig:tilted_tent_density}
\end{figure}

Figure \ref{fig:tilted_tent_density} shows how fractal the conjugate map is.
...
We derived the fractal conjugate map, and showed that a typical shadowing trajectory
is exactly the quasi-physical solution we analyzed in Section \ref{sec:quasiphysical}.
This example contrast with Section \ref{sec:sub:shadow-example}, in which the
...
\begin{figure}[H]
    ...
    \caption{The ``squashed'' tent map (\ref{eqn:tent_squashed}) for $s=0.01, 0.05, 0,2,$ and 0.5.}
...
in Figure \ref{fig:tent_squashed}, has a closed form of
\begin{equation} \label{eqn:tent_squashed}
    \widehat{\varphi^{sq}}_s(x) := \begin{cases}
    (2-s)x \quad & x < 1 \\
    (2-s)(2 - x) \quad & x \ge 1
    \end{cases}
\end{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{figure/lorenz_shadow_lorenz_shadow_density_lorenz_zmax_15_28_2.67.json.png}
    \hspace{0.005\textwidth}
    \includegraphics[width=0.48\textwidth]{figure/baseline_density_lorenz_zmax_15_28_2.67.json.png}
    \\
    \includegraphics[width=0.48\textwidth]{figure/lorenz_shadow_lorenz_shadow_density_lorenz_zmax_10_30_2.67.json.png}
    \hspace{0.005\textwidth}
    \includegraphics[width=0.48\textwidth]{figure/baseline_density_lorenz_zmax_10_30_2.67.json.png}
    \\
    \includegraphics[width=0.48\textwidth]{figure/lorenz_shadow_lorenz_shadow_density_lorenz_zmax_10_28_3.33.json.png}
    \hspace{0.005\textwidth}
