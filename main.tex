...
solutions exist \cite{grebogi, hammel}.
...
of equations, we first make a closed form approximation of the Lorenz map. The reason 
...
of the Lorenz map (see Figure \ref{fig:lorenz_map}). The cusp, around $(39,48)$, is fitted with an exponential function, and the tails are fitted with a sum of a cubic polynomial and a rational function (see supplementary material for the fitting parameters).  
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    ...
    \caption{The left column shows the empirical distribution of the shadowing solution of the Lorenz map at $\rho = 28$ (top row), $\rho = 29$ (middle row), and $\rho = 30$ (bottom row). The right column shows the probability distribution of a physical solution at the corresponding $\rho$ values of each row. The other two parameters are held at their standard values of $\sigma = 10, \beta = 8/3.$
    }
