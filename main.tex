...
\begin{equation} \label{conjugate_def}
    \varphi(h_s(x)) = h_s(\hat\varphi_s(x))\;,\quad \forall\: x
\end{equation}
...
\begin{figure}[H]
  \centering
    ...
    Upper-right: the conjugate map $\tilde{h}_s$ between the tent map (\ref{tentmap})
    and the tilted tent map (\ref{tent_tilted}), evaluated using
    Equation (\ref{conjugate_tilted}-\ref{conjugate_tilted_helper}), for the same set of $s$ as the
    upper-left plot.}
