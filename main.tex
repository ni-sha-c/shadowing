...
We conclude with a final example illustrating 
another problem with assuming the physicality of shadowing solutions: with a small change in a parameter, the stationary probability distribution of a chaotic system \emph{can} change drastically, while if shadowing solutions are physical, the change in the statistics must be small when the parameter perturbation is small. Once again, a perturbation of a tent map can be used as an example. 
