...
Thank you again for raising this question! As you point out, we have only proved that numerical solutions are not guaranteed to produce the true long-term/statistical behavior, and not that numerical solutions are always statistically inconsistent (after all, some shadowing trajectories are physical -- e.g. section 4.1). However, several references strengthen the observation of statistical inconsistency of numerics. The new references sent by Reviewer 1 that introduce clean numerical simulation, which are now cited in the revised manuscript, and the previously cited references (Sauer et al, which takes an analytical approach) in section 5 all observe that numerical solutions produce statistical/long-term inconsistencies. We have added more explanation (in \reviewerTwo{blue}) to clarify this point in section 5. 
\vspace{0.2in}
